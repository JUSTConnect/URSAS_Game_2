...
export default () => {
    ...
    const contract = new Contract('0x2dc2fAfDB7c326171671511Ea26202D81Eb1e98e', wethInterface) as any

    // const { state, send } = useContractFunction(contract, 'smartMint', { transactionName: 'Wrap' })
    const smartMint = useContractFunction(contract, 'getDataAboutCostsForRooms')
    ...
    return (
        <>
            ...
            {/* <button
                onClick={
                    () => {
                        send(1,1,{value: 1, gasLimit: 3000000}).then(c=>console.log(c))
                    }
                }
            >
                send
            ...
            <button
                onClick={
                    () => smartMint.send().then(c=>{console.log(c); console.log(smartMint.state.transaction)})
                }
            >
                send2
                { smartMint.state.transaction ? 'completed' : 'non-completed' }
            </button>
        </>
    )
}